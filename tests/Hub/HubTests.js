...
WinJSTests.HubTests = function () {
    "use strict";

    var Key = WinJS.Utilities.Key;

    var instances = 0;
    WinJSTests.HubTestsFakeControl = WinJS.Class.define(function (element, options) {
        instances++;
    });
    ...
    function getSection(index) {
        ...
        var hubSection = new WinJS.UI.HubSection(hubSectionEl, {
    ...
    function hubLoaded(hub) {
        return new WinJS.Promise(function (c, e, p) {
            function testState() {
                if (hub.loadingState === WinJS.UI.Hub.LoadingState.complete) {
                    c();
                }
            }
            testState();
            hub.addEventListener(WinJS.UI.Hub._EventName.loadingStateChanged, testState);
        });
    }

    this.testOrientation = function () {
        var hub = new WinJS.UI.Hub();
        //Default is Horizontal.
        LiveUnit.Assert.isTrue(WinJS.Utilities.hasClass(hub.element, WinJS.UI.Hub._ClassName.hubHorizontal));
        LiveUnit.Assert.isFalse(WinJS.Utilities.hasClass(hub.element, WinJS.UI.Hub._ClassName.hubVertical));

        hub.orientation = WinJS.UI.Orientation.vertical;
        LiveUnit.Assert.isTrue(WinJS.Utilities.hasClass(hub.element, WinJS.UI.Hub._ClassName.hubVertical));
        LiveUnit.Assert.isFalse(WinJS.Utilities.hasClass(hub.element, WinJS.UI.Hub._ClassName.hubHorizontal));
        ...
        hub = new WinJS.UI.Hub(undefined, {
    ...
    this.testHeaderTemplateFunction = function (complete) {
        var hub = new WinJS.UI.Hub();
    ...
    this.testHeaderTemplateBindingTemplate = function (complete) {
        var hub = new WinJS.UI.Hub();
        hub.sections = new WinJS.Binding.List(getSomeSections(5));
        hubLoaded(hub).done(function () {
            LiveUnit.Assert.areEqual("Header for section 0", hub.sections.getAt(0)._headerContentElement.textContent, "Default template");
    ...
    if (WinJS.UI.isAnimationEnabled()) {
        this.testAnimationEvents = function (complete) {
            var hubEl = document.createElement('div');
            var entranceCount = 0;
            var contentTransitionCount = 0;
            hubEl.addEventListener(WinJS.UI.Hub._EventName.contentAnimating, function (ev) {
                if (ev.detail.type === WinJS.UI.Hub.AnimationType.entrance) {
                    entranceCount++;
                } else if (ev.detail.type === WinJS.UI.Hub.AnimationType.contentTransition) {
            ...
            hubLoaded(hub).done(function () {
                ...
                hubLoaded(hub).done(function () {
                    ...
                    hubLoaded(hub).done(function () {
                        ...
                        // contentTransition
                        hub.sections = new WinJS.Binding.List();
                        hubLoaded(hub).done(function () {
    ...
    this.testLoad = function (complete) {
        var sectionCount = 5;
        instances = 0;
        var hub = new WinJS.UI.Hub(undefined, {
        ...
        hubLoaded(hub).done(function () {
            ...
            LiveUnit.Assert.areEqual(sectionCount, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection).length, "Correct HubSection dom elements found");
            complete();
        });
    };

    this.testLazyLoadFromMiddle = function (complete) {
        var hubWidth = 1024;
        var hubHeight = 768;
        ...
        var hubSectionEl = hubEl.firstChild;
        while (hubSectionEl) {
            hubSectionEl.style.width = sectionWidth + 'px';
            hubSectionEl.style.height = sectionHeight + 'px';
            hubSectionEl = hubSectionEl.nextSibling;
        }
        document.body.appendChild(hubEl);

        var hub = new WinJS.UI.Hub(hubEl, { sectionOnScreen: 5 });

        var called;
        hub.addEventListener(WinJS.UI.Hub._EventName.contentAnimating, function () {
    ...
    this.testParse = function (complete) {
        var sectionCount = 5;
        instances = 0;
        var hubEl = document.createElement('div');
        var markup = getMarkupForSomeSections(sectionCount);
    ...
    this.testIndexOfLastVisible = function (complete) {
        var startEndMargins = 80;
        var hubWidth = 1024;
        var hubHeight = 768;
        var sectionWidth = 700;
    ...
    this.testSectionOnScreen = function (complete) {
        sectionOnScreenTest(complete, false);
    }

    this.testSectionOnScreenRTL = function (complete) {
    ...
    this.testHeaderStatic = function (complete) {
        var sectionCount = 5;
        var hub = new WinJS.UI.Hub(undefined, {
            sections: new WinJS.Binding.List(getSomeSections(sectionCount))
        });
        ...
        var headerInvoked;
        hub.addEventListener(WinJS.UI.Hub._EventName.headerInvoked, function (ev) {
    ...
    this.testSectionEdits = function (complete) {
        var sectionCount = 5;
        var hub = new WinJS.UI.Hub(undefined, {
            sections: new WinJS.Binding.List(getSomeSections(sectionCount))
        });
        ...
        hubLoaded(hub).then(function () {
            ...
            LiveUnit.Assert.areEqual(sectionCount - 1, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection).length, "Pop: Correct HubSection dom elements found");

            hub.sections.push(hubSection);
            return hubLoaded(hub);
        }).then(function () {
            LiveUnit.Assert.areEqual(sectionCount, hub.sections.length, "Push: Correct # of sections found");
            LiveUnit.Assert.areEqual(sectionCount, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection).length, "Push: Correct HubSection dom elements found");
            LiveUnit.Assert.areEqual(hubSection.element, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection)[sectionCount - 1], "Push: Correct HubSection dom element found");
            ...
            LiveUnit.Assert.areEqual(sectionCount, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection).length, "Move: Correct HubSection dom elements found");
            LiveUnit.Assert.areEqual(hubSection.element, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection)[sectionCount - 2], "Move: Correct HubSection dom element found");
            ...
            LiveUnit.Assert.areEqual(sectionCount, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection).length, "Move2: Correct HubSection dom elements found");
            LiveUnit.Assert.areEqual(hubSection.element, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection)[sectionCount - 1], "Move2: Correct HubSection dom element found");

            hub.sections.pop();
            return hubLoaded(hub);
        }).then(function () {
            LiveUnit.Assert.areEqual(sectionCount - 1, hub.sections.length, "Pop2: Correct # of sections found");
            LiveUnit.Assert.areEqual(sectionCount - 1, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection).length, "Pop2: Correct HubSection dom elements found");
            ...
            LiveUnit.Assert.areEqual(sectionCount - 1, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection).length, "SetAt: Correct HubSection dom elements found");
            LiveUnit.Assert.areEqual(hubSection.element, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection)[sectionCount - 2], "SetAt: Correct HubSection dom element found");

            hub.sections.reverse();
            return hubLoaded(hub);
        }).then(function () {
            LiveUnit.Assert.areEqual(sectionCount - 1, hub.sections.length, "Reverse: Correct # of sections found");
            LiveUnit.Assert.areEqual(sectionCount - 1, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection).length, "Reverse: Correct HubSection dom elements found");
            LiveUnit.Assert.areEqual(hubSection.element, hub.element.querySelectorAll('.' + WinJS.UI.HubSection._ClassName.hubSection)[0], "Reverse: Correct HubSection dom element found");
        }).then(complete);
    };


    this.testSetGetCurrentItem = function (complete) {
        var hubWidth = 1024;
        var hubHeight = 768;
        var sectionWidth = 700;
        var sectionHeight = 768;
    ...
    this.testEmptyHub = function (complete) {
        var hubEl = document.createElement('div');
        document.body.appendChild(hubEl);
