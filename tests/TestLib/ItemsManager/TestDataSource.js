...
(function (thisNamespace) {
    ...
    if (typeof (WinJS) !== "undefined") {
        ...
        var createTestDataAdapter = function createTestDataAdapter (objects, controller, abilities) {
            ...
            function itemsFromIndexImplementation(complete, error, index, countBefore, countAfter, returnAbsoluteIndex, maxLength) {
                if (controller.communicationFailure) {
                    error(new WinJS.ErrorFromName(UI.FetchError.noResponse));
                } else {
                    ...
                    if (index >= maxLength) {
                        error(new WinJS.ErrorFromName(UI.FetchError.doesNotExist));
            ...
            function keyValid(key, error) {
                if (typeof keyToIndexMap[key] !== "number") {
                    error(new WinJS.ErrorFromName(UI.EditError.noLongerMeaningful));
                    return false;
                } else {
                    return true;
                }
            ...
            function insert(complete, error, index, data) {
                if (controller.communicationFailure) {
                    error(new WinJS.ErrorFromName(UI.EditError.noResponse));
                } else {
            ...
            function change(complete, error, key, dataNew) {
                if (controller.communicationFailure) {
                    ...
                    error(new WinJS.ErrorFromName(UI.EditError.notPermitted));
            ...
            return {
                ...
                getCount: abilities && !abilities.getCount ? undefined : function () {
                    ...
                    return createPromise("getCount", arguments, function (complete, error) {
                        ...
                        if (controller.count_NoResponse) {
                            error(new WinJS.ErrorFromName(UI.CountError.noResponse));
                        }

                        else if (controller.countUnknown) {
                            error(new WinJS.ErrorFromName(UI.CountResult.unknown));
                        }
                ...
                fulfillRandomRequest: function () {
                    if (requests.length > 0) {
                        this.fulfillRequest(Math.floor(thisNamespace.pseudorandom() * requests.length));
                    }
                },
        ...
        thisNamespace.createTestDataSource = function (objects, controller, abilities) {
