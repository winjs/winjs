...
module WinJS.Utilities {
    "use strict";

    export function _linkedListMixin(name:string) {
        var mixin = {};
        var PREV = "_prev" + name;
        var NEXT = "_next" + name;
...
module WinJS.Utilities.Scheduler {
    ...
    export class _JobNode {
        ...
        private _context;
        private _asyncOpID;
        private _owner:_OwnerToken;
        private _state;
        private _priority:Priority;

        constructor(id:number, work, priority, context, name:string, asyncOpID) {
            this._id = id;
            this._work = work;
        ...
        /// </field>
        public get owner():_OwnerToken { 
            return this._owner; 
        }
        public set owner(value:_OwnerToken) {
            this._owner && (<any>this._owner)._remove(this);
            this._owner = value;
            this._owner && (<any>this._owner)._add(this);   
        }
        ...
        private _setState(state, arg0?, arg1?) {
            if (this._state) {
    ...
    WinJS.Class.mix(_JobNode, linkedListMixin("Job"));

    export enum YieldPolicy {
        complete = 1,
        continue = 2,
        block = 3,
    }
    ...
    export class _JobInfo { 

        private _job;
        public _result;
        public _yieldPolicy:YieldPolicy;
        private _shouldYield:()=>boolean;
        private _publicApiDisabled:boolean;

        constructor (shouldYield:()=>boolean, job) {
            this._job = job;
            this._result = null;
        ...
        /// </field>
        public get shouldYield() {
            this._throwIfDisabled();
            return this._shouldYield();
        }

        public setPromise(promise:WinJS.IPromise<any>) {
    ...
    state_running.enter = function (job, shouldYield) {
        ...
        var jobInfo = new _JobInfo(shouldYield, job);
    ...
    class MarkerNode { 
        ...
        constructor(priority:Priority, name:string) {
            this.priority = priority;
            this.name = name;
        }

        public _removeJob() {
            /* Implemented by linkedListMixin */
        }

        public _insertJobBefore(node) {
            /* Implemented by linkedListMixin */
        }

        public _insertJobAfter(node) {
            /* Implemented by linkedListMixin */
        }

        public _removeMarker() {
            /* Implemented by linkedListMixin */
        }

        public _insertMarkerBefore(node) {
            /* Implemented by linkedListMixin */
        }

        public _insertMarkerAfter(node) {
            /* Implemented by linkedListMixin */
        }
    ...
    export function _dumpList(type, reverse) {
        ...
        do {
            ...
            if (current instanceof _JobNode) {
                dumpJob(current, pos);
            }
            pos++;
    ...
    export function retrieveState():string {
    ...
    export function requestDrain(priority?:Priority, name?:string) {
    ...
    export function execHigh<T>(callback:()=>T):T {
    ...
    export function createOwnerToken():IOwnerToken {
    ...
    export function schedule(work:(jobInfo: IJobInfo)=>any, priority:Priority, thisArg:any, name:string):IJob {
        ...
        name = name || "";
        return new _JobNode(jobId, work, priority, thisArg, name, asyncOpID);
    }
    ...
    if(!Scheduler.hasOwnProperty('currentPriority')) {
        Object.defineProperty(Scheduler, 'currentPriority', {
            get: getCurrentPriority,
            enumerable: true
        });
    }

    function makeSchedulePromise(priority:Priority) {
        return function<T> (promiseValue?:T, jobName?:string):Promise<T> {
    ...
    export var schedulePromiseHigh = makeSchedulePromise(Priority.high);
    export var schedulePromiseAboveNormal = makeSchedulePromise(Priority.aboveNormal);
    export var schedulePromiseNormal = makeSchedulePromise(Priority.normal);
    export var schedulePromiseBelowNormal = makeSchedulePromise(Priority.belowNormal);
    export var schedulePromiseIdle = makeSchedulePromise(Priority.idle);

    export declare var _isEmpty: boolean;
    Object.defineProperty(Scheduler, '_isEmpty', {
        get: isEmpty
    });
    
    export declare var _usingWwaScheduler;
    Object.defineProperty(Scheduler, '_usingWwaScheduler', {
        get: function () {
    ...
    export declare var _MSApp;
    Object.defineProperty(Scheduler, '_MSApp', {
        get: function () {
            return MSApp;
        },
        set: function (value) {
