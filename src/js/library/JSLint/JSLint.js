...
o.push('</div>');}}
return o.join('');
}; itself.jslint = itself; itself.edition = '2010-06-27'; return itself;
} ());
(function () { 
    WScript.StdOut.WriteLine('processing ' + WScript.Arguments(0)); 
    if (!JSLINT(WScript.StdIn.ReadAll(), { evil: true })) { 
        for(var i = 0; i <JSLINT.errors.length; ++i) {
            var e = JSLINT.errors[i]; 
            WScript.StdErr.WriteLine(WScript.Arguments(0) + '('+e.line + ',' + e.character + ') : error : ' + e.reason); 
        }
        WScript.Quit(1); 
    } 
} ());